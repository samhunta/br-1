...
export type SelectPropsExtended = SelectProps &
  ({ [prop: string]: any } & (React.ComponentPropsWithRef<'select'>))
...
const createVariantStyles = (
  theme: Theme,
  { variant, colorOverride, disabled, validated }: Partial<SelectProps>,
): SerializedStyles => {
...
const createBaseButtonStyles = (theme: Theme, { size, block, disabled }: Partial<SelectProps>): SerializedStyles => {
...
const xSelect = forwardRef(
  (
    { variant, as = 'select', size = 'md', colorOverride, block, children, customCss, disabled, validated, ...rest }: SelectPropsExtended,
    ref,
  ) => {
    const theme = useTheme()
    size = useResponsiveProp(size)

    const selectStyles = useMemo(() => createBaseButtonStyles(theme, { block, disabled, size }), [
      block,
      disabled,
      size,
      theme,
    ...
    return (
      ...
      <x.select as={as} ref={ref as any} css={flatten([selectStyles, variantStyles, customCss && css(customCss)])} {...rest}>
          {children}
      </x.select>
    )
  },
...
export const Select = memo(xSelect)
